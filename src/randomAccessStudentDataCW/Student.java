...
public class Student implements Comparable<Student> {
	 ...
	 */
	public Long writeStudentRecord(String randomfileName) throws Exception {
		// write this record out to the RandomAccess data file , it will be
		// appended to the end of the file
		// make a connection to an existing file
		RandomAccessFile randomFile = new RandomAccessFile(randomfileName, "rw");
		// find out where the end of the file is
		long location = randomFile.length();

		// seek the location
		randomFile.seek(location);

		// write out the data of this Student instance
		randomFile.writeInt(this.studentID);
		randomFile.writeDouble(this.studentBalance);
	 ...
	 * @param location
	 */
	public void rewriteStudent(String fileName, long location) throws Exception {
		// get active connection to an existing or new file
         RandomAccessFile random = new RandomAccessFile(fileName,"rw");
         random.seek(location);//seek to the location of the current student file cuz u already have the location(override
