...
public class SimulationValues {
	...
	public void run() {
		...
		while (myTimer.timeRemaining() > 0) {
			service();
			checkForNewCall();
			myTimer.tick();// takes away from myTimer
		}
	...
	void checkForNewCall() {
		...
		if (x < 100 * arrivalRate) {
			...
			int r = numGenerator.nextInt(100);// amount of time for the call
			int serviceTime = 0;
	...
	public void service() {
		...
		else if (!myIncomingCalls.isEmpty()) // calls are waiting -- get one
		{
			...
			busyTimeRemaining = nextCall.getServiceTime();// makes the next call
															// syays how many
															// minutes it is

			// Update total waiting time
			totalWaitingTime += nextCall.getArrivalTime() - myTimer.timeRemaining();
